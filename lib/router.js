...
Router._filters = {
  ...
  requireLogin: function() {
    if (this.params.user && this.params.token) {
      Meteor.logout((function(_this) {
        return function() {
          Meteor.loginWithPassword({
            id: _this.params.user
          }, _this.params.token, function(error) {
            if (error != null) {
              Router.go("/not-allowed");
            } else {
              Router.go("/top");
            }
          });
...
if(Meteor.isClient){
  ...
  Router.onBeforeAction(filters.requireLogin, {except: ["not-allowed"]});
...
PostsListController = FastRender.RouteController.extend({
  ...
  waitOn: function () {
    ...
    parsedPath = this.path.split('?')[0]
    this._terms = {
      view: parsedPath == '/' ? 'top' : parsedPath.split('/')[1],
